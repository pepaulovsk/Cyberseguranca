...
const __TWEAKS_STYLE = `
  .twk-panel{position:fixed;right:16px;bottom:16px;top:16px;z-index:2147483646;width:300px;
    height:calc(100vh - 32px);display:flex;flex-direction:column;
    transform:scale(var(--dc-inv-zoom,1));transform-origin:bottom right;
    background:rgba(250,249,247,.96);color:#29261b;
    border:.5px solid rgba(0,0,0,.12);border-radius:14px;
    box-shadow:0 1px 0 rgba(255,255,255,.6) inset,0 12px 40px rgba(0,0,0,.14);
  ...
  .twk-reopen{position:fixed;right:16px;bottom:16px;z-index:2147483646;
    ...
    padding:8px 12px;font:11px/1 ui-sans-serif,system-ui,-apple-system,sans-serif;
    letter-spacing:.03em;cursor:pointer;
    box-shadow:0 1px 0 rgba(255,255,255,.6) inset,0 4px 12px rgba(0,0,0,.1)}
  .twk-reopen:hover{background:rgba(250,249,247,1)}
  ...
  .twk-tabs-host{display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden}
  .twk-tabs{display:flex;gap:2px;padding:2px;border-radius:9px;background:rgba(0,0,0,.06);
    margin:6px 14px 0;flex-shrink:0}
  .twk-tab{flex:1;appearance:none;border:0;background:transparent;color:inherit;font:inherit;
    font-weight:500;height:24px;border-radius:7px;cursor:default;font-size:11px;padding:0 4px}
  .twk-tab[aria-selected="true"]{background:rgba(255,255,255,.9);box-shadow:0 1px 2px rgba(0,0,0,.12)}
  .twk-tab-body{padding:8px 14px 14px;display:flex;flex-direction:column;gap:10px;
    overflow-y:auto;overflow-x:hidden;min-height:0;
    scrollbar-width:thin;scrollbar-color:rgba(0,0,0,.15) transparent}
  .twk-tab-body::-webkit-scrollbar{width:8px}
  .twk-tab-body::-webkit-scrollbar-track{background:transparent;margin:2px}
  .twk-tab-body::-webkit-scrollbar-thumb{background:rgba(0,0,0,.15);border-radius:4px;
...
// ── TweaksPanel ─────────────────────────────────────────────────────────────
// Fixed shell anchored to bottom-right. Registers the host protocol
...
function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  if (!open) return (
    <>
      ...
      <button className="twk-reopen" onClick={() => setOpen(true)} aria-label="Open Design System panel">
        {title}
      </button>
    </>
  );
...
function TweakColorInput({ label, value, onChange }) {
  return (
    <div className="twk-row twk-row-h">
      <div className="twk-lbl" style={{ minWidth: 80 }}><span>{label}</span></div>
      <input type="color" className="twk-swatch" value={value}
             onChange={(e) => onChange(e.target.value)}
             style={{ width: 28, height: 22, flexShrink: 0 }} />
      <input type="text" className="twk-field" value={value}
             onChange={(e) => onChange(e.target.value)}
...
// ── TweakTabs ────────────────────────────────────────────────────────────────
// Renders a segmented tab bar + the active tab's content.
// tabs: Array<{ label: string, content: ReactElement }>
function TweakTabs({ tabs }) {
  const [active, setActive] = React.useState(0);
  return (
    <div className="twk-tabs-host">
      <div className="twk-tabs">
        {tabs.map((tab, i) => (
          <button key={i} type="button" className="twk-tab"
                  aria-selected={active === i} onClick={() => setActive(i)}>
            {tab.label}
          </button>
        ))}
      </div>
      <div className="twk-tab-body">
